...
import {
    Table, Card, Row, Col, Input, Button, Space,
    Modal, Form, App, Tooltip, Popconfirm,
    Tag, Select, Descriptions, Drawer, Dropdown, Alert, Avatar,
} from 'antd'
import {
...
const MISSING_CHIPS = [
    { label: 'Tất cả', value: '', color: '#1e88e5', emoji: '🏥' },
    { label: 'Thiếu toạ độ', value: 'coords', color: '#e53935', emoji: '📍' },
    { label: 'Thiếu khu vực', value: 'area', color: '#faad14', emoji: '🗺️' },
]
...
export default function HospitalListPage() {
    ...
    const [filters, setFilters] = useState({
        page: 1, pageSize: 10,
        sortOrder: 'newest',
        missingCoords: undefined,
        missingArea: undefined,
    })
    ...
    const autoSelectArea = async (provinceName, wardName) => {
        ...
        const pOpt = formArea.provinceOptions.find(o => normalizeArea(o.label) === provinceName)
        if (!pOpt) return
    ...
    const { data: res, isLoading } = useQuery({
        ...
        queryFn: () => hospitalApi.search({
            name: filters.name,
            province: filters.province,
            ward: filters.ward,
            missingCoords: filters.missingCoords || undefined,
            missingArea: filters.missingArea || undefined,
    ...
    // active chip
    const activeChip = filters.missingCoords ? 'coords' : filters.missingArea ? 'area' : ''

    // ── Mutations ──────────────────────────────────────────────────────────────
    const { mutate: createH, isPending: creating } = useMutation({
        mutationFn: (data) => hospitalApi.create(data),
        onSuccess: () => { message.success('Đã thêm bệnh viện'); queryClient.invalidateQueries({ queryKey: ['hospitals'] }); closeModal() },
    ...
    const { mutate: updateH, isPending: updating } = useMutation({
        ...
        onSuccess: () => { message.success('Đã cập nhật bệnh viện'); queryClient.invalidateQueries({ queryKey: ['hospitals'] }); closeModal() },
    ...
    const { mutate: deleteH, isPending: deleting } = useMutation({
        ...
        onSuccess: () => { message.success('Đã xoá bệnh viện'); queryClient.invalidateQueries({ queryKey: ['hospitals'] }); setDetailRecord(null) },
        onError: (err) => {
            const raw = err?.message || ''
            message.error(raw.includes('FK_Post_Hospital') || raw.includes('REFERENCE constraint')
                ? 'Bệnh viện đang có bài viết liên kết, không thể xoá' : raw || 'Xoá thất bại')
        },
    })
    const { mutate: bulkDeleteH, isPending: bulkDeleting } = useMutation({
        mutationFn: (ids) => hospitalApi.bulkDelete(ids),
        onSuccess: (res) => {
            message.success(`Đã xoá ${res?.data?.deleted ?? selectedRowKeys.length} bệnh viện`)
            setSelectedRowKeys([]); queryClient.invalidateQueries({ queryKey: ['hospitals'] })
        },
        onError: (err) => {
            const raw = err?.message || ''
    ...
    const { mutate: deleteAllH, isPending: deletingAll } = useMutation({
        ...
        onSuccess: (res) => {
            message.success(`Đã xoá ${res?.data?.deleted ?? 0} bệnh viện`)
            setSelectedRowKeys([]); queryClient.invalidateQueries({ queryKey: ['hospitals'] })
        },
        onError: (err) => {
            const raw = err?.message || ''
    ...
    const handleChipClick = (chipValue) => {
        ...
        setFilters(f => ({
            ...f,
            page: 1,
            missingCoords: chipValue === 'coords' ? true : undefined,
            missingArea: chipValue === 'area' ? true : undefined,
        }))
    }

    // ── Modal helpers ──────────────────────────────────────────────────────────
    const openCreate = () => {
        setEditRecord(null); form.resetFields(); formArea.reset(); setMapLink(''); setModalOpen(true)
    }
    const openEdit = (record) => {
        setEditRecord(record)
        form.setFieldsValue({ name: record.name, address: record.address, website: record.website, latitude: record.latitude, longitude: record.longitude })
        formArea.reset(); setMapLink('')
        if (record.province) autoSelectArea(record.province, record.ward)
        setModalOpen(true)
    }
    const closeModal = () => {
        setModalOpen(false); setEditRecord(null); form.resetFields(); formArea.reset(); setMapLink(''); setPendingWard(null)
    }
    ...
    const handleResolveMap = async () => {
        ...
        try {
            ...
            form.setFieldsValue({ latitude: String(latitude), longitude: String(longitude) })
            message.success(`✅ Toạ độ: ${latitude}, ${longitude}`)
            setMapLink('')
        } catch { message.error('Không lấy được toạ độ, kiểm tra lại link') }
    ...
    const handleBulkDelete = () => {
        modal.confirm({
            ...
            okText: `Xoá ${selectedRowKeys.length} bệnh viện`, okButtonProps: { danger: true },
            cancelText: 'Huỷ',
    ...
    const handleDeleteAll = () => {
        ...
        modal.confirm({
            ...
            okText: `Xác nhận xoá ${total} bệnh viện`, okButtonProps: { danger: true },
            cancelText: 'Huỷ',
            onOk: () => deleteAllH({ name: filters.name, province: filters.province, ward: filters.ward }),
        })
    }
    ...
    const handleExport = async (type) => {
        ...
        try {
            ...
            const rows = data.map((h) => ({
                name: h.name ?? '', province: h.province ?? '', ward: h.ward ?? '',
                address: h.address ?? '', latitude: h.latitude ?? '',
                longitude: h.longitude ?? '', website: h.website ?? '',
                created_at: h.createdAt ? new Date(h.createdAt).toLocaleString('vi-VN') : '',
            }))
            const ws = XLSX.utils.json_to_sheet(rows)
            const wb = XLSX.utils.book_new()
            XLSX.utils.book_append_sheet(wb, ws, 'Bệnh viện')
            XLSX.writeFile(wb, type === 'xlsx' ? 'danh_sach_benh_vien.xlsx' : 'danh_sach_benh_vien.csv', type === 'csv' ? { bookType: 'csv' } : {})
    ...
    const handleDownloadTemplate = () => {
        const ws = XLSX.utils.json_to_sheet([{ name: 'Bệnh viện Bạch Mai', province: 'Hà Nội', ward: 'Kim Liên', address: 'Số 78 đường Giải Phóng', latitude: '21.0023228', longitude: '105.8410627', website: 'https://bachmai.gov.vn' }])
        const wb = XLSX.utils.book_new()
    ...
    const handleImportFile = async (e) => {
        ...
        try {
            ...
            const items = rows.map((row) => ({
                name: String(row.name ?? '').trim(), province: String(row.province ?? '').trim() || null,
                ward: String(row.ward ?? '').trim() || null, address: String(row.address ?? '').trim() || null,
                latitude: String(row.latitude ?? '').trim() || null, longitude: String(row.longitude ?? '').trim() || null,
                website: String(row.website ?? '').trim() || null,
            }))

            // ── Phát hiện tên trùng trong file ──────────────────────────────
            const nameCount = {}
            items.forEach(({ name, province }) => {
                if (!name) return
                const key = `${name.toLowerCase()}|${(province ?? '').toLowerCase()}`
            ...
            const inFileDuplicates = [...new Set(
                items
                    .filter(({ name, province }) => {
                        ...
                        const key = `${name.toLowerCase()}|${(province ?? '').toLowerCase()}`
                        return nameCount[key] > 1
                    })
                    .map(({ name, province }) => province ? `${name} (${province})` : name)
            )]
            ...
            modal.success({
                ...
                width: inFileDuplicates.length > 0 ? 520 : 400,
                content: (
                        ...
                        {inFileDuplicates.length > 0 && (
                            <div style={{ marginTop: 12, padding: '10px 14px', background: '#fff7e6', border: '1px solid #ffd591', borderRadius: 8 }}>
                                <div style={{ fontWeight: 600, color: '#d46b08', marginBottom: 6 }}>
                                    ⚠️ {inFileDuplicates.length} tên bị trùng trong file (chỉ lần cuối được giữ):
                                </div>
                                <ul style={{ margin: 0, paddingLeft: 18, maxHeight: 160, overflowY: 'auto' }}>
                                    {inFileDuplicates.map((name) => (
                                        <li key={name} style={{ fontSize: 13, color: '#595959' }}>{name}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
        ...
        } catch (err) { message.error(err?.message || 'Nhập file thất bại') }
    ...
    const columns = [
        {
            ...
            render: (_, r) => (
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 44, height: 44, borderRadius: 10, background: '#fff2f0', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <BankOutlined style={{ color: '#e53935', fontSize: 20 }} />
                    </div>
                    <div>
                        <div style={{ fontWeight: 600, fontSize: 14, color: '#1a1a2e' }}>{r.name}</div>
                        {r.address && <div style={{ fontSize: 12, color: '#8c8c8c', marginTop: 2 }}>{r.address}</div>}
                    </div>
                </div>
            ),
        },
        ...
        },
        {
            ...
            render: (_, r) => r.latitude && r.longitude
                ? <Button type="link" size="small" icon={<EnvironmentOutlined />} style={{ padding: 0, fontSize: 12 }} onClick={() => openMap(r.latitude, r.longitude)}>Xem bản đồ ↗</Button>
                : <Tag color="red" style={{ fontSize: 11 }}>Thiếu toạ độ</Tag>,
        },
        {
            title: 'Ngày tạo',
            key: 'createdAt', width: 130,
            render: (_, r) => r.createdAt
                ? <span style={{ fontSize: 12, color: '#595959' }}>{formatDateTime(r.createdAt)}</span>
                : <span style={{ color: '#bbb', fontSize: 12 }}>—</span>,
        },
        {
            title: 'Thao tác',
            key: 'action', width: 110, fixed: 'right',
            render: (_, r) => (
                <Space size={4}>
                    <Tooltip title="Xem chi tiết">
                        <Button type="text" size="small" icon={<EyeOutlined />} style={{ color: '#1e88e5' }} onClick={() => setDetailRecord(r)} />
                    </Tooltip>
                    <Tooltip title="Chỉnh sửa">
                        <Button type="text" size="small" icon={<EditOutlined />} style={{ color: '#faad14' }} onClick={() => openEdit(r)} />
                    </Tooltip>
                    <Tooltip title="Xoá">
                        <Popconfirm title="Xoá bệnh viện?" description="Các bài đăng liên quan có thể bị ảnh hưởng." okText="Xoá" okButtonProps={{ danger: true }} cancelText="Huỷ" onConfirm={() => deleteH(r.id)}>
        ...
        <div>
            <PageHeader
                ...
                extra={[
                    ...
                    <Dropdown
                        key="export"
                        menu={{ items: [{ key: 'csv', label: 'Xuất CSV', icon: <DownloadOutlined /> }, { key: 'xlsx', label: 'Xuất Excel (.xlsx)', icon: <DownloadOutlined /> }], onClick: ({ key }) => handleExport(key) }}
                    >
                        <Button icon={<DownloadOutlined />} loading={exporting}>Xuất file ▾</Button>
                    </Dropdown>,
                    <Button key="add" type="primary" icon={<PlusOutlined />} onClick={openCreate} style={{ background: 'linear-gradient(135deg, #e53935, #b71c1c)', border: 'none', fontWeight: 600 }}>
                        Thêm bệnh viện
                    </Button>,
                ]}
            />
            ...
            <Card style={cardStyle}>
                ...
                <Row gutter={[10, 10]} style={{ marginBottom: 16 }}>
                    ...
                    <Col xs={24} sm={12} lg={7}>
                        <Input
                            ...
                            allowClear onClear={() => setNameSearch('')}
                        />
                    </Col>
                    ...
                    <Col xs={24} sm={12} lg={5}>
                        <Select
                            placeholder="Tỉnh / Thành phố" style={{ width: '100%' }}
                            loading={filterArea.loadingP} showSearch optionFilterProp="label"
                            options={filterArea.provinceOptions}
                            value={filterArea.selectedProvince?.id ?? undefined}
                            allowClear
                            onChange={filterArea.onProvinceChange}
                            onClear={() => filterArea.reset()}
                        />
                    </Col>
                    ...
                    <Col xs={24} sm={12} lg={4}>
                        <Select
                            ...
                            onChange={(val) => setFilters(f => ({ ...f, sortOrder: val, page: 1 }))}
                        />
                    </Col>
                    ...
                    <Alert
                        ...
                        message={
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                                <span style={{ fontWeight: 500 }}>Đã chọn <strong>{selectedRowKeys.length}</strong> bệnh viện</span>
                                <Space>
                ...
                <Table
                    ...
                    loading={isLoading}
                    size="middle"
                    rowClassName={(r) => (!r.latitude || !r.longitude) ? 'row-missing-coords' : ''}
                    pagination={{
                        ...
                        onChange: (page, pageSize) => { setSelectedRowKeys([]); setFilters(f => ({ ...f, page, pageSize })) },
                    }}
                    scroll={{ x: 1100 }}
            ...
            {/* ══ Drawer chi tiết ══ */}
            <Drawer
                title={<Space><BankOutlined style={{ color: '#e53935' }} /><span style={{ fontWeight: 700 }}>Chi tiết bệnh viện</span></Space>}
                open={!!detailRecord} onClose={() => setDetailRecord(null)} width={480}
                extra={
                    <Space>
                        <Button icon={<EditOutlined />} onClick={() => { openEdit(detailRecord); setDetailRecord(null) }}>Sửa</Button>
                        <Popconfirm title="Xoá bệnh viện?" description="Các bài đăng liên quan có thể bị ảnh hưởng." okText="Xoá" okButtonProps={{ danger: true }} cancelText="Huỷ" onConfirm={() => deleteH(detailRecord?.id)}>
            ...
            >
                {detailRecord && (
                    <>
                        <div style={{ textAlign: 'center', padding: '16px 0 24px', borderBottom: '1px solid #f0f0f0', marginBottom: 20 }}>
                            <div style={{ width: 64, height: 64, borderRadius: 16, background: '#fff2f0', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                                <BankOutlined style={{ color: '#e53935', fontSize: 28 }} />
                            </div>
                            <div style={{ fontWeight: 700, fontSize: 16, color: '#1a1a2e' }}>{detailRecord.name}</div>
                            {detailRecord.province && <Tag color="blue" style={{ marginTop: 8 }}><EnvironmentOutlined /> {detailRecord.province}</Tag>}
                        </div>

                        <Descriptions column={1} size="middle" labelStyle={{ color: '#8c8c8c', fontWeight: 500, width: 140 }}>
                            <Descriptions.Item label="Địa chỉ">{detailRecord.address || <span style={{ color: '#bbb' }}>Chưa cập nhật</span>}</Descriptions.Item>
                            ...
                            <Descriptions.Item label="Ngày tạo">{formatDateTime(detailRecord.createdAt) || <span style={{ color: '#bbb' }}>—</span>}</Descriptions.Item>
                            <Descriptions.Item label="Người tạo">
                                {detailRecord.createdByName
                                    ? <span>{detailRecord.createdByName}{detailRecord.createdByUsername && <span style={{ color: '#8c8c8c' }}> @{detailRecord.createdByUsername}</span>}</span>
                                    : <span style={{ color: '#bbb' }}>—</span>}
                            </Descriptions.Item>
                            <Descriptions.Item label="Cập nhật lần cuối">{formatDateTime(detailRecord.updatedAt) || <span style={{ color: '#bbb' }}>—</span>}</Descriptions.Item>
                            <Descriptions.Item label="Người cập nhật">
                                {detailRecord.updatedByName
                                    ? <span>{detailRecord.updatedByName}{detailRecord.updatedByUsername && <span style={{ color: '#8c8c8c' }}> @{detailRecord.updatedByUsername}</span>}</span>
                                    : <span style={{ color: '#bbb' }}>—</span>}
                            </Descriptions.Item>
                        </Descriptions>
                        ...
                        {detailRecord.latitude && detailRecord.longitude && (
                            <div style={{ marginTop: 20 }}>
                                <div style={{ fontWeight: 600, fontSize: 13, color: '#1a1a2e', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <EnvironmentOutlined style={{ color: '#e53935' }} />Vị trí trên bản đồ
                                </div>
            ...
            {/* ══ Modal tạo / sửa ══ */}
            <Modal
                title={<Space><BankOutlined style={{ color: '#e53935' }} /><span style={{ fontWeight: 700 }}>{editRecord ? 'Chỉnh sửa bệnh viện' : 'Thêm bệnh viện mới'}</span></Space>}
                open={modalOpen} onCancel={closeModal} footer={null} width={660} destroyOnClose
            >
                <Form form={form} layout="vertical" onFinish={handleSubmit} style={{ marginTop: 16 }}>
                    <Form.Item label={<span style={{ fontWeight: 600 }}>Tên bệnh viện</span>} name="name" rules={[{ required: true, message: 'Nhập tên bệnh viện' }]}>
                        <Input placeholder="VD: Bệnh viện Đa khoa Phố Nối" size="large" />
                    </Form.Item>
                    <Form.Item label={<span style={{ fontWeight: 600 }}>Địa chỉ cụ thể</span>} name="address">
                    ...
                    <div style={{ background: '#f8f9fa', borderRadius: 10, padding: '14px 16px', marginBottom: 16 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 12, color: '#1a1a2e' }}>
                            <EnvironmentOutlined style={{ color: '#1e88e5', marginRight: 6 }} />Toạ độ GPS
                        </div>
                        <div style={{ background: '#e8f4fd', borderRadius: 8, padding: '10px 12px', marginBottom: 12, border: '1px dashed #90caf9' }}>
                            <div style={{ fontSize: 12, color: '#1e88e5', fontWeight: 500, marginBottom: 6 }}>🔗 Dán link Google Maps → tự động điền</div>
                            <Space.Compact style={{ width: '100%' }}>
                                <Input placeholder="https://maps.app.goo.gl/..." value={mapLink} onChange={(e) => setMapLink(e.target.value)} onPressEnter={handleResolveMap} allowClear />
                                <Button type="primary" loading={resolving} icon={<EnvironmentOutlined />} onClick={handleResolveMap} style={{ background: '#1e88e5', border: 'none' }}>Lấy toạ độ</Button>
                            </Space.Compact>
                        </div>
                        <Row gutter={10}>
                            <Col span={12}>
                                <Form.Item label="Vĩ độ (Latitude)" name="latitude" style={{ marginBottom: 0 }} rules={[{ pattern: /^-?\d+(\.\d+)?$/, message: 'Nhập số hợp lệ' }]}>
                                    <Input placeholder="VD: 20.9801" />
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item label="Kinh độ (Longitude)" name="longitude" style={{ marginBottom: 0 }} rules={[{ pattern: /^-?\d+(\.\d+)?$/, message: 'Nhập số hợp lệ' }]}>
                    ...
                    <div style={{ background: '#f8f9fa', borderRadius: 10, padding: '14px 16px', marginBottom: 20 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 12, color: '#1a1a2e' }}>
                            <EnvironmentOutlined style={{ color: '#e53935', marginRight: 6 }} />Khu vực hành chính
                        </div>
                        <Row gutter={10}>
                            <Col span={12}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                        ...
                        <Button type="primary" htmlType="submit" loading={creating || updating} style={{ background: 'linear-gradient(135deg, #e53935, #b71c1c)', border: 'none', fontWeight: 600 }}>
            ...
            <style>{`
                .row-missing-coords td { background: #fff9f0 !important; }
                .row-missing-coords:hover td { background: #fff3e0 !important; }
            `}</style>
        </div>
    )
}